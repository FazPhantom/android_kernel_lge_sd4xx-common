...
static bool lge_moisture_detection(struct wcd_mbhc *mbhc, int switch_device)
{
	...
	if (!psy || !psy->get_property)
	    return 0;
	psy->get_property(psy, POWER_SUPPLY_PROP_MOISTURE, &pval);
	...
	pr_info("%s: result = %s , pval : %d\n", __func__, result?"true":"false", pval.intval);
				...
				struct snd_soc_jack *jack, int status, int mask)
{
	...
	if ((mask == WCD_MBHC_JACK_MASK) &&
	    ...
	    if (lge_moisture_detection(mbhc, switch_device) && !strcmp(mbhc->sdev.name,LGE_SWITCH_NAME_AUX)) { /* Both Moisture and AUX Case */
